# ================================================================================
# APPLICATION INS - SYSTEME INTEGRE DE SUIVI DES ACTIVITE
# Organisation: Institut National de la Statistique (INS)
# ================================================================================

# Liste des packages nécessaires
#packages <- c("shiny", "shinydashboard", "bs4Dash", "shinyWidgets", "DT", "shinyjs", "dplyr", "ggplot2", "plotly", "readr", "lubridate", "tidyr")
...
load_users <- function() {
  if(file.exists(nom_fichier_users)) {
    ...
    users_db_default <- data.frame(
      ...
      direction = c("Direction Générale", "Direction des Enquètes et Recensement", 
                    ...
                    "Direction des Enquètes et Recensement",
                    "Direction des Statistiques Economiques",
                    "Direction des Statistiques Démographiques et Sociales",
                    "Direction des Statistiques et des Analyses Economiques"),
...
# --- TYPES D'ACTIVITéS ---
types_activites <- c("Sélectionnez le type", "Enquète", "Recensement", 
            ...
            fluidRow(
              ...
              column(
                ...
                textInput("nom_activite", "Nom de l'activité (Unique) :",
                         placeholder = "Ex: Enquète harmonisée sur les conditions de vie")
              )
            ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$login_btn, {
    # Charger les utilisateurs Ã  jour
    ...
    if(nrow(user_match) == 1) {
      ...
      # Afficher les menus selon le rele
      ...
      # Seul le Directeur Général peut accéder Ã  l'administration
 ...
 #       "Role : ", auth$user_info$role, "\n",
  ...
  output$etapes_form_ui <- renderUI({
    ...
    lapply(seq_along(etapes_config), function(i) {
      ...
      # Vérifier si un fichier existe déjÃ 
  ...
  output$status_TDR <- renderUI({
    if(isTRUE(input$val_TDR)) {
      tags$span(style = "color: #00a65a; font-weight: bold; font-size: 11px;", "â ON")
    } else {
      tags$span(style = "color: #dd4b39; font-weight: bold; font-size: 11px;", "â OFF")
    }
  })
  
  # Logique séquentielle des étapes - SIMPLIFIé
  # Observer uniquement pour les étapes 2 Ã  10 (pas l'étape 1 !)
  ...
  # Fonction pour filtrer les données selon le role
  ...
  output$table_users <- renderDT({
    ...
    datatable(
      ...
      colnames = c("Utilisateur", "Role", "Direction", "Service", "Bureau", "Actions")
    )
  })
  ...
  observeEvent(input$add_user_btn, {
    tryCatch({
      ...
      showModal(modalDialog(
        ...
        fluidRow(
          column(6, selectInput("new_user_role", "Role :",
  ...
  observeEvent(input$confirm_add_user, {
    ...
    # Vérifier si l'utilisateur existe déjÃ 
    if(input$new_user_name %in% users()$user_name) {
      showNotification("Cet utilisateur existe déjÃ  !", type = "error", duration = 3)
      return()
    }
  ...
  observeEvent(input$edit_user, {
    ...
    if(nrow(user_data) > 0) {
      ...
      showModal(modalDialog(
        ...
        fluidRow(
          column(6, selectInput("edit_user_role", "Role :",
                               choices = c("Directeur général", "Direction_Centrale", "RESPONSABLE"),
                               selected = user$role)),
  ...
  observeEvent(input$confirm_edit_user, {
    ...
    # Trouver l'index de l'utilisateur Ã  modifier
  ...
  observeEvent(input$delete_user, {
    ...
    showModal(modalDialog(
      ...
      paste0("Ãtes-vous sÃ»r de vouloir supprimer l'utilisateur '", input$delete_user, "' ?"),
      br(), br(),
